...
专家 checkpoint 未覆盖时仍会降低分数。每个 checkpoint 通过 `evidence_expectations` 关联自己的运行时证据，避免一次财务查询错误地替客户、抵押等风险点解锁文本优化。框架再根据轨迹区分后续动作：没有对应工具时新增工具或 MCP；工具明确返回无记录时扩展数据源或新增工具；有工具但未调用时优化调用触发；参数错误时优化调用语义；运行时失败时修工具或环境；已有对应证据但未形成风险点时再优化 skill/reference。评分不会因为缺工具而放宽，文本优化也不会替工具故障背锅。

checkpoint 应保持原子化，一个 checkpoint 只对应一个可独立判断的风险机制。例如专家分别讨论“盈利波动”和“盈利质量”时，应拆成两项，避免一项命中、另一项遗漏却被全有或全无地评分。清洗结果还会写入 `tool_coverage` 和工具可支持/总 checkpoint 数量，供分层切分与运行前诊断使用。
...
配置会按工具覆盖度和难度进行确定性的分层训练集、验证集和测试集划分。对于这种 rubric-only 专家数据，框架先用种子候选运行训练集预检：只有文本可修复失败和少量已满足回归样本进入 GEPA 优化池；缺少工具或运行证据的样本保留为诊断与工具建设清单。预检调用从总预算中扣除。优化结束后，框架自动在留出的测试集上比较种子与最佳候选，并将结果保存在运行目录的 `final_test/` 中。
